...
Ten tools in four groups. `artifact-*` owns one artifact's lifecycle, `workspace-*` owns the registry, `codegen-*` writes files into your own project.
...
| `files` | `import` | object | `{path: text}`, ~100 KB max, text only |
| `zipUploadId` | `import` | uuid | From `artifact-get_zip_upload_url` |
| `framework` | `empty` | enum | `html` \| `react` — **only these two**; defaults to `html` |
...
`l2c` output is always TypeScript. `name` is accepted but ignored by generation types (they name themselves) — rename later with `artifact-update_metadata`.
...
`{ sessionId, playgroundUrl, previewUrl, name, gitRemoteUrl, access, expiresAt, nextSteps }` plus `fileCount` and `skippedFiles` for `import`. **A read-write git token comes back in this same response** — don't call `artifact-get_git_token` after creating.
...
**Returns:** `{ success, sessionId, status: "generating" | "ready" | "failed", progress (0–100 while generating), name, playgroundUrl, previewUrl, error (when failed), nextStep }`
...
Lists your team's artifacts. **No parameters.** Each row includes the session id to pass to the `artifact-*` tools.
...
| `uiLibrary` | no | enum | `mui`, `antd`, `shadcn` |
